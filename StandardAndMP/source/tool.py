...
class Control():
    def __init__(self):
        ...
        net = QNetwork_rainbow(c.GRID_Y_LEN, c.GRID_X_LEN, self.numPlants, device='cuda:0', use_zombienet=False, use_gridnet=False).to("cuda:0")
        self.numPlants = 4
        self.agent = RainbowAgent(net, buffer, n_iter=-1, batch_size=200, numPlants=self.numPlants)
    ...
    def main(self):
        ...
        for episode in train_progress_bar:
            ...
            state = torch.cat([torch.tensor(self.state.PlantGrid.flatten(), dtype=torch.float32), torch.tensor(self.state.ZombieGrid.flatten(), dtype=torch.float32), torch.tensor([self.state.menubar.sun_value], dtype=torch.float32), torch.tensor(self.state.menubar.getAvailableMoves()[:self.numPlants], dtype=torch.float32)])
            ...
            while not self.done:
                ...
                nextState = torch.cat([torch.tensor(self.state.PlantGrid.flatten(), dtype=torch.float32), torch.tensor(self.state.ZombieGrid.flatten(), dtype=torch.float32), torch.tensor([self.state.menubar.sun_value], dtype=torch.float32), torch.tensor(self.state.menubar.getAvailableMoves()[:self.numPlants], dtype=torch.float32)])
    ...
    def mainRainbow(self):
        ...
        for episode in train_progress_bar:
            ...
            # print(self.game_info[c.LEVEL_NUM])
            state = torch.cat([torch.tensor(self.state.PlantGrid.flatten(), dtype=torch.float32), torch.tensor(self.state.ZombieGrid.flatten(), dtype=torch.float32), torch.tensor([self.state.menubar.sun_value], dtype=torch.float32), torch.tensor(self.state.menubar.getAvailableMoves()[:self.numPlants], dtype=torch.float32)])
            self.agent.s_0 = state
            
            while not self.done:
                
                if self.agent.buffer.burn_in_capacity() < 1:
                    prevScore = self.state.score
                    mask = self.state.getActionMask()
                    action = self.agent.take_step(mask, "explore")
                    stepAgent(action)
                    
                    s_1 = torch.cat([torch.tensor(self.state.PlantGrid.flatten(), dtype=torch.float32), torch.tensor(self.state.ZombieGrid.flatten(), dtype=torch.float32), torch.tensor([self.state.menubar.sun_value], dtype=torch.float32), torch.tensor(self.state.menubar.getAvailableMoves()[:self.numPlants], dtype=torch.float32)])
                    currentScore = self.state.score
                    reward = currentScore - prevScore
                    
                    self.agent.buffer.append(self.agent.s_0, action, reward, self.done, s_1)
                    # self.agent.push(state.cpu(), action, reward, nextState.cpu(), self.done, mask)
                    self.agent.s_0.copy_(s_1)
                    
                else:
                    self.agent.epsilon = self.agent.threshold.epsilon(episode)
                    prevScore = self.state.score
                    mask = self.state.getActionMask()
                    action = self.agent.take_step(mask, "explore")
                    # print(action)
                    ...
                    reward = currentScore - prevScore
                    # print(reward)
                    if total_steps % network_update_frequency == 0:
                        # print(f"Pushing: {state}")
                        # print("Pushing to buffer")
                        self.agent.update
                        
                    # Sync networks
                    if total_steps % network_sync_frequency == 0:
                        self.agent.target_network.load_state_dict(self.agent.network.state_dict())
                        self.agent.sync_eps.append(episode)

                    if total_steps % Noisy_Update ==0:
                        self.agent.network.reset_noise()
                        self.agent.target_network.reset_noise()
                    ...
                    # self.agent.epsilon = max(self.agent.min_epsilon, self.agent.epsilon * self.agent.epsilon_decay)
            mean_rewards_list.append(self.state.score)
            np.save("./pretrainedModels/Rainbow3000NetSyncFreq/PVZScoresRainbow.pth", mean_rewards_list)
