...
class Level(tool.State):
    ...
    def initPlay(self, card_list):
        self.state = c.PLAY
        if self.bar_type == c.CHOOSEBAR_STATIC:
            self.menubar = menubar.MenuBar(card_list, self.map_data[c.INIT_SUN_NAME], self.current_time)
        else:
    ...
    def getActionMask(self):
        ...
        availableActions = np.tile(self.menubar.getAvailableMoves()[:self.numPlants], c.GRID_X_LEN * c.GRID_Y_LEN)
