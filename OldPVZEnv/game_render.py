...
def render(render_info):
    ...
    bg = pygame.image.load("assets/PVZBack.png").convert_alpha()

    clock = pygame.time.Clock()
    cell_size = 75
    bgOffset = 75
    offset_border = bgOffset + 60
    offset_x = 175
    ...
    while render_info:
        ...
        screen.blit(bg, (bgOffset, bgOffset))
        
        # The grid
        for i in range(config.LANE_LENGTH+1):
            pygame.draw.line(screen, (0, 0, 0), (offset_border + offset_x + i * cell_size, offset_border), 
                (offset_border + offset_x + i * cell_size, offset_border + cell_size * (config.N_LANES)), 1)
        for j in range(config.N_LANES+1):
            pygame.draw.line(screen, (0, 0, 0), (offset_border + offset_x, offset_border + j * cell_size), 
                (offset_border + offset_x + cell_size * (config.LANE_LENGTH), offset_border + j * cell_size), 1)
        ...
        for lane in range(config.N_LANES):
            for zombie_name, pos, offset in frame_info["zombies"][lane]:
                ...
                screen.blit(zombie_sprite[zombie_name], (offset_border + offset_x + cell_size * (pos + offset) - zombie_sprite[zombie_name].get_width(),
            ...
            for plant_name, pos in frame_info["plants"][lane]:
                ...
                screen.blit(plant_sprite[plant_name], (offset_border + offset_x + cell_size * pos, 
            ...
            for projectile_name, pos, offset in frame_info["projectiles"][lane]:
                ...
                screen.blit(projectile_sprite[projectile_name], (offset_border + offset_x + cell_size * (pos+offset) - projectile_sprite[projectile_name].get_width(), 
        ...
        screen.blit(cooldowns_text, (400, 600))
        time = myfont.render('Time: '+ str(frame_info["time"]), False, (0, 0, 0))
        screen.blit(time, (1100, 100))
...
if __name__ == "__main__":
    ...
    if agent_type == "Reinforce":
        env = PlayerV2(render=False, max_frames = 1e6 * config.FPS)
    ...
    if agent_type == "AC":
        env = TrainerAC3(render=False, max_frames = 1e6*config.FPS)
        agent = ACAgent3(
                input_size=env.num_observations(),
                possible_actions=env.get_actions()
        )
    ...
    if agent_type == "Keyboard":
        env = PlayerV2(render=True, max_frames = 1e6*config.FPS)
        agent = KeyboardAgent()
    print(type(env))
    env.play(agent)
    render_info = env.get_render_info()
    print(len(render_info))
    render(render_info)
